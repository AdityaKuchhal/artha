...
export default function DashboardPage() {
  ...
  return (
    <div className="page fade-up">
      ...
      {isEmpty && (
        <div className="card" style={{ padding: '40px 32px', display: 'flex', alignItems: 'center', gap: '32px' }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '13px', color: 'var(--text3)', lineHeight: '1.6', marginBottom: '20px' }}>
              Get started by adding your jobs and logging shifts, or connect your bank to sync transactions.
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <a href="/dashboard/cards" style={{ textDecoration: 'none' }}>
                <button className="btn btn-primary"><Briefcase size={13} /> Add a Job</button>
              </a>
              <a href="/dashboard/cards" style={{ textDecoration: 'none' }}>
                <button className="btn btn-ghost"><Link2 size={13} /> Connect Bank</button>
              </a>
            </div>
          </div>
          <div style={{
      ...
      <div className="grid-3">
        <StatCard
          ...
          badge={hasShifts
            ? { label: '+' + formatCurrency(weeklyEarnings?.total_earnings || 0) + ' this week', type: 'green' }
                ...
                <Bar dataKey="amount" radius={[6, 6, 0, 0]}>
                  {barData.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={entry.amount > 0
                        ? entry.isToday ? 'var(--accent)' : 'var(--green)'
                        : 'var(--bg3)'}
                    />
                  ))}
                </Bar>
...
function RecentTransactions({ hasTransactions }: { hasTransactions: boolean }) {
  ...
  if (!hasTransactions || !transactions.length) return (
        ...
        <div key={txn.id} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '14px', fontWeight: 700, color: getCategoryColor(txn.ai_category),
            }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 600, color: txn.amount < 0 ? 'var(--green)' : 'var(--text)' }}>
