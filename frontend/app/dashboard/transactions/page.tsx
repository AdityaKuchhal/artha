...
const CATEGORIES = [
  'All', 'FOOD_AND_DRINK', 'TRANSPORTATION', 'RENT_AND_UTILITIES',
  'LOAN_PAYMENTS', 'TRANSFER_OUT', 'TRANSFER_IN',
  'GENERAL_MERCHANDISE', 'PERSONAL_CARE', 'TRAVEL',
]
...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    api.transactions.list()
      .then(t => { setTransactions(t); setFiltered(t) })
      .catch(console.error)
    ...
    <div className="page fade-up">
      ...
      <div className="grid-3">
        {[
          ...
          <div key={s.label} className="card" style={{ padding: '18px 22px' }}>
            <div style={{ fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '8px' }}>
              {s.label}
            </div>
            <div style={{ fontSize: '26px', fontWeight: 700, color: s.color, letterSpacing: '-0.5px' }}>
              {s.value}
            </div>
          </div>
        ))}
      ...
      <div className="card" style={{ padding: '16px 20px' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-ghost" onClick={sync} disabled={syncing} style={{ marginLeft: 'auto' }}>
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={4} style={{ textAlign: 'center', color: 'var(--text3)', padding: '32px' }}>Loading...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={4} style={{ textAlign: 'center', color: 'var(--text3)', padding: '32px' }}>No transactions found.</td></tr>
            ) : filtered.map(txn => (
              <tr key={txn.id}>
                <td style={{ color: 'var(--text3)', fontSize: '12px' }}>{formatDate(txn.date)}</td>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{
                      ...
                      fontSize: '12px', fontWeight: 700, color: getCategoryColor(txn.ai_category),
                    }}>
                    ...
                    <div>
                      <div style={{ fontSize: '13px', fontWeight: 500, color: 'var(--text)' }}>
                        {txn.merchant_name}
                        {txn.is_subscription && (
                          <span className="badge-blue" style={{ marginLeft: '8px', fontSize: '10px' }}>sub</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <td style={{ textAlign: 'right', fontWeight: 600, color: txn.amount < 0 ? 'var(--green)' : 'var(--text)' }}>
