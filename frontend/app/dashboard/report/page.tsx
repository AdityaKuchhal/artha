    ...
    <div className="page fade-up">
      ...
      {isRunning && (
        <div className="card" style={{ textAlign: 'center', padding: '48px 32px' }}>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
            <Clock size={12} />
      ...
      {report && jobStatus === 'complete' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{
            ...
            Report complete · {elapsedSeconds}s · {report.transactions_processed ?? 0} transactions analyzed
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {report.budget_alerts!.map((alert, i) => (
                <div key={i} style={{
                  ...
                  <AlertTriangle size={15} style={{ color: alert.severity === 'critical' ? 'var(--red)' : '#f59e0b', flexShrink: 0, marginTop: '1px' }} />
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: alert.severity === 'critical' ? 'var(--red)' : '#f59e0b' }}>
                      {alert.category.replace(/_/g, ' ')} — {alert.percent_used}% of budget used
                    </div>
                    <div style={{ fontSize: '12px', color: 'var(--text3)', marginTop: '2px' }}>{alert.message}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {(report.anomalies?.length ?? 0) > 0 && (
            <div className="card">
              ...
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr><th>Merchant</th><th>Amount</th><th>Date</th><th>Reason</th></tr>
                  </thead>
                  <tbody>
                    {report.anomalies!.map((a, i) => (
                      <tr key={i}>
                        <td style={{ color: 'var(--text)', fontWeight: 500 }}>{a.merchant}</td>
                        <td style={{ color: 'var(--red)', fontWeight: 600 }}>{formatCurrency(a.amount)}</td>
          ...
          <div className="card" style={{ borderColor: 'rgba(59,130,246,0.2)' }}>
            ...
            <p style={{ color: 'var(--text3)', fontSize: '13px', lineHeight: '1.7', whiteSpace: 'pre-line' }}>
              {report.report}
            </p>
          </div>
          ...
          <div className="grid-2">
            <div className="card">
              <div style={{ fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>Total Spent</div>
              <div style={{ fontSize: '28px', fontWeight: 700, color: 'var(--text)' }}>{formatCurrency(report.total_spent ?? 0)}</div>
            </div>
            <div className="card">
              <div style={{ fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>Daily Burn Rate</div>
              <div style={{ fontSize: '28px', fontWeight: 700, color: 'var(--text)' }}>
                {formatCurrency(report.daily_burn_rate ?? 0)}
                <span style={{ fontSize: '14px', fontWeight: 400, color: 'var(--text3)' }}>/day</span>
              </div>
            </div>
          </div>
      ...
      {jobStatus === 'idle' && (
        <div className="card" style={{ textAlign: 'center', padding: '64px 32px' }}>
          ...
          <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text)', marginBottom: '8px' }}>Run your financial report</div>
          <div style={{ fontSize: '13px', color: 'var(--text3)' }}>
