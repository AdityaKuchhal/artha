...
class TestUpdateShift:
    ...
    def test_update_shift_not_found(self, test_client):
        client, _ = test_client
        with patch(
            "backend.api.routes.shifts.update_shift",
            side_effect=ValueError("Shift not found or unauthorized"),
        ):
            response = client.patch(
                "/shifts/nonexistent",
                json={
...
class TestDeleteShift:
    def test_delete_shift_success(self, test_client):
        ...
        response = client.delete(
            "/shifts/shift-uuid-001", headers={"authorization": FAKE_AUTH_HEADER}
        )
    ...
    def test_delete_shift_not_found(self, test_client):
        ...
        response = client.delete("/shifts/nonexistent", headers={"authorization": FAKE_AUTH_HEADER})
...
class TestGetShifts:
    def test_get_shifts_returns_list(self, test_client):
        ...
        mock_sb.execute.return_value = MagicMock(
            data=[make_shift(), make_shift({"id": "shift-002"})]
        )
...
class TestEarningsSummary:
    def test_earnings_summary_monthly(self, test_client):
                ...
                make_shift(
                    {
                        "hours_worked": 6.0,
                        "earnings": 103.20,
                        "jobs": {"name": "Tim Hortons", "hourly_rate": 17.20, "color": "#3b82f6"},
                    }
                ),
                make_shift(
                    {
        ...
        response = client.get(
            "/shifts/earnings/summary?period=monthly", headers={"authorization": FAKE_AUTH_HEADER}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_earnings_summary_invalid_period(self, test_client):
        client, _ = test_client
        response = client.get(
            "/shifts/earnings/summary?period=yearly", headers={"authorization": FAKE_AUTH_HEADER}
        )
