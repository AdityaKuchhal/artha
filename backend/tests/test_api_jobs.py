...
class TestGetJobs:
    ...
    def test_get_jobs_returns_list(self, test_client):
        client, mock_sb = test_client
        mock_sb.execute.return_value = MagicMock(data=[make_job(), make_job({"id": "job-002", "name": "Walmart"})])
...
class TestDeleteJob:
    ...
    def test_delete_job_success(self, test_client):
        ...
        response = client.delete("/jobs/job-uuid-001",
                                  headers={"authorization": FAKE_AUTH_HEADER})
    ...
    def test_delete_job_not_found(self, test_client):
        client, mock_sb = test_client
        mock_sb.execute.return_value = MagicMock(data=[])
        with patch("backend.income.jobs.delete_job", side_effect=ValueError("Job not found or unauthorized")):
